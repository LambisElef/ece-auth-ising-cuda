...
__global__ void spin(int *G_d, double *w, int *newG_d, int n) {
    ...
    const int G_length = blockSize + 4;  // = blockDim.x+4;

    // Calculates the first Atomic Spin index. Note: n/blockDim.x=sqrt(gridDim.x).
    int index = (blockIdx.x/(n/blockSize))*n*blockSize + (blockIdx.x%(n/blockSize))*blockSize;
    ...
    double weightSum;
    for (int i=index+threadIdx.x/blockSize*n+threadIdx.x%blockSize; i<(index+(blockSize-1)*n+blockSize) && i<n*n; i+=((i%n)%blockSize+blockDim.x)/blockSize*n + ((i%n)%blockSize+blockDim.x)%blockSize - (i%n)%blockSize) {
        weightSum = 0;

        // Calculates weight contribution for each neighboring Atomic Spin and sums it.
        weightSum += w[0] * G[((i/n)%blockSize + 2 - 2)*G_length +(i%n)%blockSize + 2 - 2];
        weightSum += w[1] * G[((i/n)%blockSize + 2 - 2)*G_length +(i%n)%blockSize + 2 - 1];
        weightSum += w[2] * G[((i/n)%blockSize + 2 - 2)*G_length +(i%n)%blockSize + 2];
        weightSum += w[3] * G[((i/n)%blockSize + 2 - 2)*G_length +(i%n)%blockSize + 2 + 1];
        weightSum += w[4] * G[((i/n)%blockSize + 2 - 2)*G_length +(i%n)%blockSize + 2 + 2];

        weightSum += w[5] * G[((i/n)%blockSize + 2 - 1)*G_length +(i%n)%blockSize + 2 - 2];
        weightSum += w[6] * G[((i/n)%blockSize + 2 - 1)*G_length +(i%n)%blockSize + 2 - 1];
        weightSum += w[7] * G[((i/n)%blockSize + 2 - 1)*G_length +(i%n)%blockSize + 2];
        weightSum += w[8] * G[((i/n)%blockSize + 2 - 1)*G_length +(i%n)%blockSize + 2 + 1];
        weightSum += w[9] * G[((i/n)%blockSize + 2 - 1)*G_length +(i%n)%blockSize + 2 + 2];

        weightSum += w[10] * G[((i/n)%blockSize + 2)*G_length +(i%n)%blockSize + 2 - 2];
        weightSum += w[11] * G[((i/n)%blockSize + 2)*G_length +(i%n)%blockSize + 2 - 1];
        // w[12] is not contributing anything. It's the current Atomic Spin.
        weightSum += w[13] * G[((i/n)%blockSize + 2)*G_length +(i%n)%blockSize + 2 + 1];
        weightSum += w[14] * G[((i/n)%blockSize + 2)*G_length +(i%n)%blockSize + 2 + 2];

        weightSum += w[15] * G[((i/n)%blockSize + 2 + 1)*G_length +(i%n)%blockSize + 2 - 2];
        weightSum += w[16] * G[((i/n)%blockSize + 2 + 1)*G_length +(i%n)%blockSize + 2 - 1];
        weightSum += w[17] * G[((i/n)%blockSize + 2 + 1)*G_length +(i%n)%blockSize + 2];
        weightSum += w[18] * G[((i/n)%blockSize + 2 + 1)*G_length +(i%n)%blockSize + 2 + 1];
        weightSum += w[19] * G[((i/n)%blockSize + 2 + 1)*G_length +(i%n)%blockSize + 2 + 2];

        weightSum += w[20] * G[((i/n)%blockSize + 2 + 2)*G_length +(i%n)%blockSize + 2 - 2];
        weightSum += w[21] * G[((i/n)%blockSize + 2 + 2)*G_length +(i%n)%blockSize + 2 - 1];
        weightSum += w[22] * G[((i/n)%blockSize + 2 + 2)*G_length +(i%n)%blockSize + 2];
        weightSum += w[23] * G[((i/n)%blockSize + 2 + 2)*G_length +(i%n)%blockSize + 2 + 1];
        weightSum += w[24] * G[((i/n)%blockSize + 2 + 2)*G_length +(i%n)%blockSize + 2 + 2];
...
__global__ void check(int *G, int *newG, int n, int *same) {
    ...
    for (int i=index+threadIdx.x/blockSize*n+threadIdx.x%blockSize; i<(index+(blockSize-1)*n+blockSize) && i<n*n; i+=((i%n)%blockSize+threadsNum)/blockSize*n + ((i%n)%blockSize+threadsNum)%blockSize - (i%n)%blockSize)
...
void ising(int *G, double *w, int k, int n) {
    ...
    for (int i=0; i<k; i++) {
        ...
        spin<<<n/blockSize*n/blockSize,threadsNum>>>(G_d, w_d, newG_d, n);
        gpuErrchk( cudaPeekAtLastError() );
        gpuErrchk( cudaDeviceSynchronize() );

	//check<<<n/blockSize*n/blockSize,threadsNum>>>(G_d, newG_d, n, same_d);
        //gpuErrchk( cudaPeekAtLastError() );
        //gpuErrchk( cudaDeviceSynchronize() );
